...
public class IppProxyServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        logger.info("Received get request on IPP Proxy. {}", String.format("printer: %s", req.getPathInfo()));
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        logger.info("Received post request on IPP Proxy. {}", String.format("printer: %s mappings: %s", req.getRequestURI(), req.getHttpServletMapping().toString()));
        ...
        IppPacket generatedPacket = new IppPacket(requestPacket.getOperation(), requestPacket.getRequestId(), operationGroup);
        ...
        for (Map.Entry<String, List<String>> entry : responseHeaders.entrySet()) {
            if (StringUtils.isNotBlank(entry.getKey())) {
                if (!(entry.getKey().equalsIgnoreCase("Content-Type") ||
                    ...
                    logger.info("setting header {}", String.format("key: %s value: %s", entry.getKey(), StringUtils.join(entry.getValue(), ";")));
